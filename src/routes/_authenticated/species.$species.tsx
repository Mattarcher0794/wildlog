    ...
    <main className="min-h-screen pb-[calc(7.5rem+env(safe-area-inset-bottom))]">
      ...
      <div className="relative -mt-6">
        <div className="relative h-60 w-full overflow-hidden">
          ...
          <span className="absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-background to-transparent" />
        </div>
        <Link
          to="/life-list"
          aria-label="Back to life list"
          className="absolute left-5 top-4 grid h-10 w-10 place-items-center rounded-full bg-background/70 backdrop-blur"
        >
      ...
      <section className="mx-auto -mt-10 max-w-md px-5">
        ...
        >
          ...
          {entry.latest.description && (
            <p className="mt-5 rounded-2xl bg-card p-4 text-sm leading-relaxed text-foreground/85">
              {entry.latest.description}
            </p>
          )}
        </motion.div>
        ...
        <ul className="space-y-3">
          {timeline.map((s, i) => (
            <motion.li
              ...
              className="card-journal flex gap-3.5 bg-card p-3"
            >
              {s.image_url ? (
                <img
                  src={s.image_url}
